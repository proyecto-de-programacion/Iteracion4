...
#define N_CMD 15 /*!< Maximum number of commands allowed*/
/**
...
struct _Command{
  ...
  char link_name[MAX_NAME+1]; /*!< name of link*/
};

/**
 * @brief Transformation of a long command to string
 */
char *cmd_to_str[N_CMD] = {"No command", "Unknown", "Exit", "Next", "Back", "Left","Right","Roll","Pickup", "Drop","Move","Inspect", "Turnon", "Turnoff", "Open"};
/**
 * @brief Transformation of a short command to string
 */
char *short_cmd_to_str[N_CMD] = {"","","e","n","b","l","r","rll","p","d","m","i","ton","toff","op"};
...
char *command_get_link_name(Command *pc){
  if(!pc){
    return NULL;
  }
  return pc->name;
  ...
  if(fgets(input,CMD_LENGHT,stdin) != NULL){
    if(strcmp(input,"\n") != 0){
      ...
      if(toks != NULL){
        ...
        else if (!strcasecmp(action,"ton") || !strcasecmp(action,"turnon")){
          pc->cmd = TURNON;
          strcpy(pc->name,cmd_name);
        }
        else if (!strcasecmp(action,"toff") || !strcasecmp(action,"turnoff")){
          pc->cmd = TURNOFF;
          strcpy(pc->name,cmd_name);
        }
        else if (!strcasecmp(action,"op") || !strcasecmp(action,"open")){
          
          pc->cmd = OPEN;

          toks = strtok(cmd_name, " \n");
          if(toks != NULL){

            strcpy(pc->link_name, cmd_name);
            toks = strtok(cmd_name, " \n");
            toks = strtok(cmd_name, " \n");
