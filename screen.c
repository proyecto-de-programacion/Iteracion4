...
#define ROWS 40  /*!< Maximum number of rows */
#define COLUMNS 100 /*!< Maximum number of columns */
#define TOTAL_DATA (ROWS * COLUMNS) + 1  /*!< Maximum number of total data */

#define BG_CHAR '~'  /*!< bg_char replacement */
#define FG_CHAR ' ' /*!< fg_char replacement */
#define PROMPT " prompt:> "  /*!< prompt replacement */

#define ACCESS(d, x, y) (d + ((y) * COLUMNS) + (x)) /*!< type of access */

/**
 *   @brief Area strcture
 *
 *   Stores relevant information about the area the game is in
 *
 */

struct _Area{
  int x/*!< the x coordinate */, y/*!<the y coordinate*/, width /*!< width coordinate*/, height; /*!< height coordinate*/
  char *cursor; /*!< Screen pointer cursor*/
};
...
/*     Private functions    */
/****************************/
int  screen_area_cursor_is_out_of_bounds(Area* area); /*!< bounds of the cursor*/
void screen_area_scroll_up(Area* area); /*!< scroll up of the area*/
void screen_utils_replaces_special_chars(char* str);  /*!< mickey special tools, we will need them laters*/
...
void screen_paint(){
  ...
  if (__data){
    ...
    for (src=__data; src < (__data + TOTAL_DATA - 1); src+=COLUMNS){
      ...
      for (i=0; i<COLUMNS; i++){
	         if (dest[i] == BG_CHAR){
	            printf("\033[0;47;41m%c\033[0m", dest[i]); /* fg:blue(34);bg:blue(44) */
	         }else{
	            printf("\033[0;47;00m%c\033[0m", dest[i]); /* fg:black(30);bg:white(47)*/
	           }
           }
      printf("\n");
    }
